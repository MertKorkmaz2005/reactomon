...
class PokemonCard extends React.Component {
  ...
  attack = () => {
    ...
    let damagePercentage = Math.floor(this.state.hp / this.props.pokemonHP * 100);
    ...
    if (this.state.caughtOrRan === false) {
      buttons = (
        <>
          <button
            onClick={this.attack}
            className="pokemonCard__button pokemonCard__button--attack"
          >
            Attack
          </button>
          <button
            onClick={this.catch}
            className="pokemonCard__button pokemonCard__button--catch"
          >
            Catch
          </button>
        </>
    ...
    return (
      <article className="pokemonCard">
        <header className="pokemonCard__header">
          ...
          <div className="pokemonCard__labelWrapper">{types}</div>
        </header>
        ...
        <footer className="pokemonCard__footer">{buttons}</footer>
      </article>
    );
  }
}
